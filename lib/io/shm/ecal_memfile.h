...
namespace eCAL
{
	...
	**/
	...
	{
	public:
		//enum for lock_type configuration
		 ...
		 * @brief Constructor.
		**/
		CMemoryFile(lock_type lock_choice);
		...
		**/
		 ...
		 * @return  true if it succeeds, false if it fails.
		**/
		// Added timeout for when a rw-lock is used. Defaults to zero when a mutex is used.
		bool ReleaseReadAccess(const int64_t timeout_ = 0);
